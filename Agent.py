...
# def run_timestamps(env,model_topo, model_route,args,ReplyBuffer,horizen,device):
    ...
    for ts in range(timestamps):  # timestamps=1 ，一个样本目前是一个时间步
        ...
        # env.selected_matrix = env.step(action_topo_single,1)#action与environment交互
        ...
        # new_state, reward ,done = env.step(action_route,2)
        # 输出 reward 及其各部分
        # print("Reward:", reward)
...
class MyRouteAgent(nn.Module):
    ...
    def __init__(self,in_shape,hidden_d=100, share_backbone=False,rnd_hidden_dim=128,num_choices=8,num_heads=1):
         # 父类构造函数
            ...
            for dst in range(8):
                if src != dst:
                    ...
                    if routes:  # 如果有路径
                        ...
                        action_index = Categorical(route_probs).sample()  # 根据概率选择路径的索引
                        actions[src][dst] = action_probs[0][dst][action_index]
                    else:
...
#                   horizon,lamda, gamma, epsilon, n_epochs, batch_size, lr,c1, c2, c3,device):
    ...
    for iteration in range(max_iterations):
        ...
        for actor in range(n_actors):
            ...
            while ReplyBuffer.episode_num < 16:  # 模拟 do while 
                # run_timestamps( env, model_topo, model_route, args, ReplyBuffer,
                #         horizon, device)
                run_timestamps( env, model_topo, model_route, args, ReplyBuffer, socket_connection,
                        horizon, device)
            ...
            for epoch in range(n_epochs):
                ...
                # route_log_prob = route_actions_batch+0.5
                ...
                route_ratio = route_actions_batch+0.5
                route_ratio =route_ratio.mean(dim=-1)
...
def testing_loop(env, model_topo, model_route,args, n_episodes,horizon, device):
    # n_episodes=5
